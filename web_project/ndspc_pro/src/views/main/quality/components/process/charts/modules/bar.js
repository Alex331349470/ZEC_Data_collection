		...
		tooltip: {
			...
			formatter: function (params, ticket, callback) {
				...
				for (var i = 0; i < params.length; i++) {
					...
					htmlStr += seriesName + '：' + value + chartData.units[i] + '<br/>';//图例名称：y轴值圆点后面显示的文本
					htmlStr += '</div>';
				}
				return htmlStr;
			}
		...
		series: [
			{
				...
				data: chartData.yAxis_data.data_A,
				symbolSize: 9, //设置拐点大小
				itemStyle: {
					color: color[0]
				},
				...
				type: 'bar',
				name: chartData.names.length > 2 ? chartData.names[1] : null,
				data: chartData.yAxis_data.data_B,
				symbolSize: 9, //设置拐点大小
				itemStyle: {
					color: color[1]
				},
				...
				data: chartData.yAxis_data.data_C,
				symbolSize: 9, //设置拐点大小
				symbol: 'circle',
				color: color[2], //设置颜色
				lineStyle: {
				...
				label: {
					show: true,
					position: 'top', // 显示位置
					formatter: `{c} ${chartData.units[2]}` // 格式化显示内容，{c} 表示数值
				},
				yAxisIndex: 1
			}, {
				type: 'line',
				name: chartData.names[3],
				data: chartData.yAxis_data.data_D,
				color: color[3], //设置颜色
