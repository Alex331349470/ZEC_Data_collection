...
import {materialQCTestItem} from '@/api/quality/incoming'
  ...
  function refreshData(params) {
    ...
    switch (props.row.name) {
      ...
      case '物料编码不良数量柏拉图':
        ...
        break
      case '检测项目合格率':
        getMaterialQCTestItem(params, '检测项目合格率')
        break
      case '检测项目不良数量柏拉图':
        getMaterialQCTestItem(params, '检测项目不良数量柏拉图')
        break
    }
  }
  // 来料合格率处理方法
  ...
  function getTestItemPlato(val) {
    ...
    let rate = []
    let purpose = []
    const data = val.chartData.materialQCTestItem.testItemPlato
    const parmas = val.params
    if(parmas.quantityCalcSwitch) { //按批次数量
  ...
  function getTestItemRate(val) {
    ...
    let rate = []
    let purpose = []
    const data = val.chartData.materialQCTestItem.testItemRate
    const parmas = val.params
    if(parmas.quantityCalcSwitch) { //按批次数量
  ...
  async function getMaterialQCTestItem(val, type) {
    const res = await materialQCTestItem({input: val.params})
    switch(type) {
      case '检测项目合格率':
