...
import {SpcProductSelectConnectControlValue} from '@/api/spc/control'
...
const defaultForm = config.control_connect_input
...
function getInfo(row) {
  ...
  newForm.analyzeNum = row.analyze_num
  newForm.materialCode = row.material_code
  newForm.materialType = row.material_type
  newForm.propertyType = row.property_type
  newForm.testItem = row.analyze_column
  newForm.selectedTime = 5
  Object.assign(searchForm, { ...newForm })
  handleSearch()
}
async function handleSearch () {
  loading.value = true
  let res = await SpcProductSelectConnectControlValue({input: searchForm})
  const data = res.data?.spcProductSelectConnectControlValue || {}
